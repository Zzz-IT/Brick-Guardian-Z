...
一个无感、自动化、优先支持 KernelSU 的防砖模块，并在遇到启动异常（Bootloop）时自动进行精细化救砖和模块禁用。本模块同时内置了对旧版 `magisk-brick-guardian` 遗留影响的清理与逐项恢复能力。
...
- **三重救砖策略**：
  ...
  3. **大范围禁用**：当连续失败达到阈值（默认 6 次），将不分青红皂白禁用所有非白名单模块，以确保手机最终能够开机。
